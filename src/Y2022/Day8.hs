...
viewDistanceFromLeft point grid = helper 0 (get point grid) (goLeft point) grid
  where helper :: Int -> Int -> Point -> Grid -> Int
...
countVisible grid =  length [(x, y) | x  <- [0.. getLength grid - 1 ],
                             y <- [0.. getWidth grid - 1], isVisible (x,y) grid]

maxScore :: Grid -> Int
maxScore grid = maximum [scenicScore (x,y) grid |  x  <- [0.. getLength grid - 1 ],
                             y <- [0.. getWidth grid - 1]]
