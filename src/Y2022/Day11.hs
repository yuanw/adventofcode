...
import Data.Attoparsec.Text (
    ...
    many1,
    parseOnly,
    string, double, signed, sepBy, decimal, char
    )

type Idx = Int
type Val = Int
data Monkey = Monkey { _index :: Int, _items :: [Val], _op :: Val -> Val,
                      _divisible :: Val, _trueThrow :: Idx , _falseThrow :: Idx,
                      _count :: Int}
$(makeLenses ''Monkey)
instance Show Monkey where
    show m = "Monkey " ++ show (_index m ) ++ " inspected items " ++ show (_count m) ++ " times. :" ++ show (_items m)
eval :: Int ->  [Monkey] ->[Monkey]
eval i monkeys  = let monkey = monkeys !! i in if null (view items monkey) then monkeys else let item = head (view items monkey)
