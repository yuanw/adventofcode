...
follow s = if distant s < 2 then s else helper s
  where helper :: RopeState -> RopeState
...
visited movements = foldl (\ (s, points) m -> let ns = move m s
                                                  np = map snd ns in (last ns,  points ++ ns ) ) (empty, [empty]) movements
-- ..##..
-- ...##.
-- .####.
-- ....#.
